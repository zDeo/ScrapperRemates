-- Migración 004: vista con matching histórico por rangos de año (solo Karcal)
-- Lógica: 1) año exacto → 2) ±1 año → 3) año más cercano disponible
...
select
  ...
  -- Precio consolidado - mejor disponible (solo Karcal)
  coalesce(
    -- año exacto
...
left join lateral (
  -- Precio de mercado Chileautos: promedio de años dentro del rango ±1 año
  select round(avg(pm2.precio_mercado))::bigint as precio_mercado
  from   precios_mercado pm2
  where  pm2.marca  = v.marca
    and  pm2.modelo = v.modelo
    and  pm2.anio between coalesce(v.anio, 0) - 1 and coalesce(v.anio, 9999) + 1
) pm on true;
