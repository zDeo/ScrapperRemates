-- Migración 006: fix precio_remate_promedio opción 3
-- Antes: promedio de TODOS los años disponibles (incorrecto)
-- Ahora: promedio del AÑO MÁS CERCANO disponible (igual que hist_ref_precio)
...
select
  ...
  -- 3. Año más cercano disponible (solo Karcal) — el AÑO
  (select h.anio
   from vehiculos h
   join remates rh on rh.id = h.remate_id
   join empresas_remate eh on eh.id = rh.empresa_id
  ...
  -- 3. Año más cercano disponible (solo Karcal) — el PRECIO de ese año específico
  ...
  -- Precio consolidado: exacto → ±1 año → año más cercano (NO promedio global)
  coalesce(
    -- Opción 1: año exacto
    ...
    (select round(avg(h.precio_final))::bigint from vehiculos h
     ...
     where h.marca=v.marca and h.modelo=v.modelo
       ...
       and h.vendido=true and h.precio_final is not null
       and eh.nombre='Karcal'),
    -- Opción 3: año más cercano disponible (precio de ESE año, no promedio global)
    (select round(avg(h2.precio_final))::bigint
     from vehiculos h2
     join remates rh2 on rh2.id = h2.remate_id
     join empresas_remate eh2 on eh2.id = rh2.empresa_id
  ...
  -- Precio de mercado Chileautos (por vehículo, con filtros km+transmisión+año±1)
  pm.precio_mercado,
  pm.precio_min    as precio_mercado_min,
  pm.precio_max    as precio_mercado_max,

  -- Precio sugerido de compra = 80% del mejor promedio histórico disponible
  case
    when coalesce(
      (select round(avg(h.precio_final))::bigint from vehiculos h
       join remates rh on rh.id = h.remate_id
  ...
  -- Margen estimado (solo si hay promedio exacto o ±1 año — no con referencia lejana)
  case
...
left join lateral (
  select precio_mercado, precio_min, precio_max
